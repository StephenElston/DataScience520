...
- the **alternative hypotheses** $H_1$ is a competing statement that suggest something to the contrary and suggests some interesting **effect**, such as    
    - a drug is significnly difference from a **control**     
    - these two groups came from different underlying distributions    
...
- Can be a very large number of alternatives   
...
1. We can think of $H_0$ and $H_1$ as two assumptions *about the population* opposed to each other and competing for attention
2. Under the assumption made by $H_0$, we expect the statistic(s) we collect to follow a certain distribution (or some other condition)
3. We can then collect a sample of data (our **evidence**), compute the statistic(s) and ask what the probability is to get the statistic(s) we got if $H_0$'s assumptions really hold, we call this probability the **p-value**
4. we compare the p-value to some pre-determined threshold $\alpha$, also called the **type-I error** threshold, usually $\alpha = 0.01$ or $\alpha = 0.05$
5. if p-value $< \alpha$ we **reject $H_0$**, otherwise we **fail to reject $H_0$** and conclude that $H_0$ still holds    
  - We avoid saying that we "accept" $H_0$
  - p-value $< \alpha$ does **not** mean we accept the alternative
  - May be able to and conclude $H_a$ in certain cases  
...
- Decide on a significance level (probability cutoff): e.g. 0.1, 0.05, and 0.01    
...
- Failing to rejecting the null hypothesis can occur for several reasons       
  ...
  - There is not enough evidence for the **size of the effect**    
...
- In pain language, the **power is the probability of getting a positive result when the null hypothesis is not true**      
...
1. Most typically, determine how big a sample is required to have high probability of rejecting the null hypothesis given the expected effect size     
...
2. Or, determine how big an effect is required for a fixed sample size to have hight probabiliyt of rejecting the null hypothesis    

3. Finally, determine the how different cutoff values change the probability of detecting an effect.   
